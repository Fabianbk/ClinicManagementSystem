...
export default function EditPatientPage({
  ...
  useEffect(() => {
    async function loadPatient() {
      try {
        ...
        setNationalId(data.nationalId ? formatNationalId(data.nationalId) : "");
        ...
        setMaritalStatus((data.maritalStatus as MaritalStatus) || "SINGLE");
        ...
        setBloodGroupAbo((data.bloodGroupAbo as BloodGroupAbo) || "UNKNOWN");
        setBloodGroupRh((data.bloodGroupRh as BloodGroupRh) || "UNKNOWN");
        setTreatmentRights((data.treatmentRights as TreatmentRights) || "PAY_DIRECT");
        ...
        setMobileNumber(data.mobileNumber ? formatPhoneNumber(data.mobileNumber) : "");
        setEmail(data.email || "");

        setOriginalDomicile(data.originalDomicile || "");
        setBirthPlace(data.birthPlace || "");
        setEducation(data.education || "");
        setHouseholdStatus((data.householdStatus as HouseholdStatus) || "");
        ...
        if (data.contactPersons && data.contactPersons.length > 0) {
          setEmergencyContacts(
            data.contactPersons.map((c) => ({
              ...
              mobileNumber: c.mobileNumber ? formatPhoneNumber(c.mobileNumber) : "",
            }))
          );
        } else {
          setEmergencyContacts([
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      toast.success("อัปเดตข้อมูลผู้ป่วยสำเร็จเรียบร้อยแล้ว!");
      setIsDirty(false);
      startTransition(() => {
  ...
  return (
    <div className="space-y-6 pb-20 font-body text-clinic-ink">
      <PageHeader
        ...
        title={`แก้ไขข้อมูลผู้ป่วย: ${fullname || `HN-${patientId}`}`}
        subtitle={`HN: P-${String(patientId).padStart(5, "0")} · ปรับปรุงข้อมูลทั่วไป ข้อมูลติดต่อ และประวัติเวชระเบียน`}
        actions={
